        ...
        <div class="row" style="width: 100vw">
            <div class="col-sm-6">
                <div v-if="this.$store.state.loggedUser.length != 0" style="float: left">
                    ...
                    <router-link to="/table">Informações</router-link>
                </div>
            ...
            <div class="col-sm-6">
                <form class="form-inline" v-if="this.$store.state.loggedUser.length != 0" style="float: right">
                    <a class="navbar-brand" style="float: left"> {{this.$store.state.loggedUser.name}}</a>
                    <button class="btn btn-outline-success" type="button" @click="logout()">Terminar
                        Sessão</button>
                </form>
            </div>
        </div>
